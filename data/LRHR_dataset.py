...
class LRHRDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # text zoom only
        if 'HR_height' in self.opt:
            img_HR = cv2.resize(img_HR, (HR_size, self.opt['HR_height']), interpolation = cv2.INTER_CUBIC)
        # modcrop in the validation / test phase
        if self.opt['phase'] != 'train' and 'HR_height' not in self.opt:
        ...
        if self.paths_LR:
            ...
            # text zoom only
            if 'HR_height' in self.opt:
                img_LR = cv2.resize(img_LR, (HR_size // scale, self.opt['HR_height'] // scale), interpolation = cv2.INTER_CUBIC)
